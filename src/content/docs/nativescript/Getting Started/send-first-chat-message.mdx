...
description: Step-by-step guide to sending your first chat message using ConnectyCube NativeScript Chat SDK - what exactly to do when you want to build a chat.
head:
  - tag: title
    content: Send first chat message guide | ConnectyCube NativeScript SDK
sidebar: 
...
If you’re planning to build a new app, we recommend starting with one of our [code samples apps](/nativescript/getting-started/code-samples/) as a foundation for your client app.    
...
<TabItem label='npm'>
```bash
npm install --save nativescript-connectycube
```
</TabItem>
<TabItem label='yarn'>
```bash
yarn add nativescript-connectycube
```
</TabItem>
</Tabs>
...
Follow the [Chat SDK documentation](/nativescript/messaging) to enrich your app and engage your users even further!