...
description: Simplify user authentication in your ReactNative app with our definitive API guide. Fortify your app's defenses and protect user data effectively.
head:
  - tag: title
    content: ReactNative Authentication and Users APIs | ConnectyCube
sidebar: 
...
To perform CIdP login, the same ConnectyCube [User Login API](/reactnative/authentication-and-users/#upgrade-session-token-user-login) is used. You just use existing login request params to pass your external user token: 
